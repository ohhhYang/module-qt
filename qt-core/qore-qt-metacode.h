...
class T {
   ...
   public:
	 ...
	 //printd(5, "deleteBlocker() %s returning %s\n", metaObject()->className(), PARENTTYPE::parent() ? "true" : "false");

	 if (PARENTTYPE::parent() || externally_owned) {
	    if (!obj_ref) {
	       obj_ref = true;
	 ...
	 //printd(5, "detach() this=%08p (%s) qore_obj=%08p obj_ref=%d parent=%08p\n", this, metaObject()->className(), qore_obj, obj_ref, PARENTTYPE::parent());
	 if (obj_ref) {
	    obj_ref = false;
	    ExceptionSink xsink;
	    qore_obj->deref(&xsink);
