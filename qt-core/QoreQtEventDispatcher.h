...
class QoreQtEventDispatcher {
   protected:
	 ...
	 discard(dispatch_event_intern(qore_obj, m, qclass, data, &xsink), &xsink);
      ...
      DLLLOCAL static void dispatch_event(QoreObject *qore_obj, const QoreMethod *m) {
	 ExceptionSink xsink;

	 discard(dispatch_event_intern(qore_obj, m, 0, &xsink), &xsink);
      }

      DLLLOCAL static void dispatch_event(QoreObject *qore_obj, const QoreMethod *m, QoreListNode *args, ExceptionSink *xsink) {
	 discard(dispatch_event_intern(qore_obj, m, args, xsink), xsink);
      }

      DLLLOCAL static bool dispatch_event_bool(QoreObject *qore_obj, const QoreMethod *m, QoreClass *qclass, AbstractPrivateData *data) {
	 ExceptionSink xsink;
	 ...
	 return (m && m->isUser()) ? m : 0;
      ...
      DLLLOCAL static AbstractQoreNode *dispatch_event_intern(QoreObject *qore_obj, const QoreMethod *m, QoreClass *qclass, class AbstractPrivateData *data, ExceptionSink *xsink) {
	 ...
	 return qore_obj->evalMethod(*m, *args, xsink);
      ...
      DLLLOCAL static AbstractQoreNode *dispatch_event_intern(QoreObject *qore_obj, const QoreMethod *m, const QoreListNode *args, ExceptionSink *xsink) {
	 // call event method
	 return qore_obj->evalMethod(*m, args, xsink);
      }
};
